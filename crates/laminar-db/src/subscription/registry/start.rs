...
use super::{head_sequence, MvUpdate, StreamLogInner, SubscriptionOpenError};
...
/// future entry skipped. A cursor behind the earliest retained entry (or any
/// cursor when retention is disabled) fails closed with the earliest retained
...
pub(super) fn cursor_after_sequence(
    ...
    // `head_sequence` reports `next_sequence` when the log was fully evicted,
    // which is the truthful earliest retained sequence in that case.
    let head = head_sequence(inner);
    if cursor < head {
        return Err(SubscriptionOpenError::SequencePruned {
            requested,
            earliest_retained: head,
        });
    }
    Ok(cursor)
}
